<x-app-layout>
    <x-slot name="header">Edit Voucher</x-slot>

    <div class="max-w-5xl mx-auto" x-data="{
        code: '{{ $voucher->code }}',
        amount: {{ $voucher->amount }},
        type: '{{ $voucher->type }}',
        min: {{ $voucher->min_purchase_amount }},
        quota: {{ $voucher->quantity }},
        date: '{{ $voucher->end_date ? $voucher->end_date->format('Y-m-d') : date('Y-m-d') }}'
    }">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-xl font-black text-[#251A4A]">Edit Voucher</h2>
                ...
                <form action="{{ route('vouchers.update', $voucher) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input id="code" type="text" name="code" x-model="code" class="mt-1 block w-full rounded-xl border-gray-200 focus:border-[#4C3494] focus:ring-[#4C3494] uppercase font-mono font-bold bg-gray-50" required placeholder="CONTOH: MERDEKA45">
            ...
            <!-- Kuota -->
            <div x-data="{ unlimited: {{ $voucher->quantity === null ? 'true' : 'false' }} }">
                <x-input-label for="quantity" value="Batas Pemakaian (Kuota)" />
                <div class="flex items-center gap-4 mt-1">
                    <div class="flex-1 relative">
                        <x-text-input id="quantity" class="block w-full pl-10" type="number" name="quantity" x-bind:disabled="unlimited" value="{{ $voucher->quantity }}" placeholder="Contoh: 100" min="1" />
